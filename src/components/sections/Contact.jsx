...
const Contact = () => {
  ...
  // Cấu hình URL backend PHP
  const API_URL = 'http://localhost/FIDT-LANDINGPAGE/backend/contact_handler.php';
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.errors) {
          ...
          setServerError(result.message || 'An error occurred while sending your message.');
        }
      }
    } catch (error) {
