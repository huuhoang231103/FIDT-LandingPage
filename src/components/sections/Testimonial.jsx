...
const Testimonial = () => {
    const testimonials = [
        // [DỮ LIỆU GIỮ NGUYÊN như bạn đã đưa]
        {
            name: 'Sarah Johnson',
            position: 'CEO, TechCorp',
            image: 'https://images.unsplash.com/photo-1494790108755-2616b332c8ee?w=150&h=150&fit=crop&crop=face',
            rating: 5,
    ...
    const [intervalTime] = useState(4000);
    ...
    }, [isHovered, testimonials.length, isMobile, intervalTime]);
                ...
                >
                    ...
                    >
                        <div 
                            ...
                            style={{ 
                                transform: `translateX(-${isMobile ? currentIndex * 100 : (currentIndex / 2) * 100}%)` 
                            }}
                        >
                            {isMobile ? (
                                ...
                                Array.from({ length: totalSlides }, (_, slideIndex) => (
                                    <div key={slideIndex} className="w-full flex-shrink-0 grid grid-cols-2 gap-8 px-4">
                                        {testimonials.slice(slideIndex * 2, slideIndex * 2 + 2).map((testimonial, index) => (
                                            <div key={slideIndex * 2 + index} className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                                                <div className="flex items-center mb-6">
                                                    {[...Array(testimonial.rating)].map((_, i) => (
                                                        <Star key={i} className="w-5 h-5 text-blue-500 fill-current" />
                                                    ))}
                        ...
                        {Array.from({ length: totalSlides }, (_, index) => (
                            <button
                                ...
                                className={`w-3 h-3 rounded-full transition-all duration-300 ${
                                    index === (isMobile ? currentIndex : Math.floor(currentIndex / 2))
